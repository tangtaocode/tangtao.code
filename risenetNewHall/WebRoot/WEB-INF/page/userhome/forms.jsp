...
function changeUrl(obj,aurl,approveItemGUID,insGuid,userId){
	aurl=aurl+"&querystring=sb.Guid='"+insGuid+"'";
						...
						<tr>
							<s:iterator value="formList" status="form">
								<s:if test="#form.getCount()==1">
									<td><iframe id="showform" name="showform" style="height: 700px;width: 100%; border-top: 1px solid #333;" src="<s:property value='url'/>&approveItemGUID=<s:property value='approveItemGUID'/>&insGuid=<s:property value='appInstanceGuid'/>&userId=<s:property value="#session.loginUser.guid"/>&querystring=sb.Guid='<s:property value='appInstanceGuid'/>'"></iframe></td>
								</s:if>
							</s:iterator>
						</tr>
				</table>	
...
var LODOP; //声明为全局变量
	function PreviewMytable(){//alert(document.frames["showform"].document.body.table.innerHTML);
		...
		LODOP.ADD_PRINT_HTM(0,0,"100%","100%",styles+strHtml);	
