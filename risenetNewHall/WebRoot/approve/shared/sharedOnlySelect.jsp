...
<%@page import="net.risesoft.dwr.SearchSharedOnlyData"%>
...
<script
	src='<%=request.getContextPath()%>/dwr/interface/SearchSharedOnlyData.js'></script>
	<script type="text/javascript">
<%
   SearchSharedOnlyData.findHtEmpInfo(request);
   SearchSharedOnlyData.findEmployeeInfo(request);
%>
	function saveData(){
	var gcxh = document.getElementById("gcxh").value;
		...
		for(var i=0;i<guids.length;i++){
            if(guids[i].checked){
            	cert_id = guids[i].parentNode.childNodes[0].value.split(";")[1];
            	guid =guids[i].parentNode.childNodes[0].value.split(";")[0];
                break;
            }
        }
		if(guid==""){
		...
		SearchSharedOnlyData.findEmpShared(sharedGuid,guid,insGuid,gcxh,type,cert_id,initData);
	}
	function cancelSelectData(){
		...
		var sharedGuid = "<%=request.getParameter("sharedGuid")%>";
		var guid = "";
		SearchSharedOnlyData.cancelSelect(sharedGuid, guid,insGuid,initData);
	}
...
<form name="SharedConfigForm" method="post" action="/approve/shared/sharedOnlySelect.jsp" target="myself" >
				...
				for(int a=0;a<result.size();a++)
		  		{
					%>
						<tr>
							<td nowrap="nowrap">
								<input type="radio" name="selectGuid" value=<%=((Map)result.get(a)).get("ID_NUMBER")+";"+((Map)result.get(a)).get("alt_cert_id")  %> />
							</td> 
					<%
					for(int i=1;i<list.size();i++){
						%>
