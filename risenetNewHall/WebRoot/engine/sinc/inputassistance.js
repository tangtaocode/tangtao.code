...
function confirmSubmit(frmObj)
{
	...
	//质量安全监管处 提示到窗口办理
    //if(document.getElementById("pingbi_submit")!=null){
	//	alert("请打印表单并带齐材料到窗口办理。");
	//}
	
	//施工许可;申请事项--提前开工核准选中，则应急工程批复文件文号必填
	try{
		if(document.getElementById("fwsq_tqkg").checked==true)
			if(document.getElementById("fwsq_wenhao").value==""){
				alert("应急工程批复文件文号 不能为空");
				document.getElementById("fwsq_wenhao").focus();
				return false;
			}
	}catch(e){};
        ...
        if(_button != null && _button.type=="submit" && _button.form != null)
        {
            if(!_button.form.onsubmit)
            {
                _button.form.onsubmit = function()
                {
                    ...
                    if(eval(document.getElementById('otherValid'))){
                		if(!otherValidInput()) return false;
                	}
