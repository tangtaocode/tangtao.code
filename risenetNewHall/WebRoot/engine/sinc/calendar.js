...
document.write("<iframe name=meizzCalendarIframe id=meizzCalendarIframe scrolling=no frameborder=0 width=144 height=193 src=blank.htm></iframe></div>");
...
function writeIframe()
{
    ...
    "    <table id=tableDay height=110 width=140 border=0 cellspacing=1 cellpadding=0>";
    ...
    "    onfocus='this.blur()' style='meizz:expression(this.disabled=parent.WebCalendar.thisYear==1000);width:20px'><input"+
    "    onfocus='this.blur()' name=prevMonth title='向前翻 1 月&#13;快捷键：←' onclick='parent.prevM()' style='width:20px' type=button value='&lt;'>"+
    ...
    "    onfocus='this.blur()' style='width:20px'><input name=nextYear title='向后翻 1 年&#13;快捷键：↓' onclick='parent.nextY()' type=button value='&gt;&gt;'"+
...
function calendar() //主调函数
{
    ...
    try{WebCalendar.iframe.tableWeek.style.cursor = WebCalendar.drag ? "move" : "default";}catch(e){}
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        hiddenCalendar();
		if(WebCalendar.callback != null){try{eval(WebCalendar.callback + "('" + returnValue + "')")}catch(e){}}
    }
}
