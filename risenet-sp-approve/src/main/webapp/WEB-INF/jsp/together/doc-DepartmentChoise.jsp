...
<script type="text/javascript">
	...
	function addTab(){
		$('#tabdiv').tabs('add',{
			title:'部门',
			content: '<iframe id="iframeId" name="iframeName" scrolling="auto" frameborder="0"  src="${ctx}/sp/document/choiceTab?principalType=1" style="width:100%;height:380px;"></iframe>'  // 新内容的URL
		});
	}
	...
	function addSelectedNodes(){
		var child=null;
		zTreeObj=window.frames["iframeName"].zTreeObj;child=iframeName;
	...
	function deleteReceiveObj(principalGuid,rowIndex){
		...
		zTreeObj=window.frames["iframeName"].zTreeObj;
	...
	$('#submit1').click(function() {
		var result = genUserChoice();
		var sponsorGuid = "";
		if (result == "") {
			alert("请选择用户");
			return;
		}
		parent.send(result);//发送
		closeCurWindow(parent.frameID,'close');
	});
