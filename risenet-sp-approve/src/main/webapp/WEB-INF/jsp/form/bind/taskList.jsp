...
<title>工作流中间件-表单绑定的任务选取列表</title>
...
<div class="easyui-layout" data-options="fit:true,border:true">
		<div data-options="region:'center',title:'表单与流程绑定(${procDefName})',border:true">
	...
	grid.datagrid({
		...
		columns : [[ {
			...
			formatter : function(cellvalue, rowObject, rowIndex) {
				var s = '';
				s = '<a href="javascript:void(0);" onclick="permManage(\'' + rowObject.taskDefKey +  '\',\'' + rowObject.taskDefName + '\');">绑定表单管理</a>';
				s += '&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="permManage1(\'' + rowObject.taskDefKey +  '\',\'' + rowObject.taskDefName + '\');">绑定电子表单管理</a>';
				return s;
			}
		} ]]
	});
...
{
	window.location.href="${ctx}/form/bind/list?procDefKey=${procDefKey}&processDefinitionId=${processDefinitionId}&procDefName=${procDefName}&taskDefKey="+taskDefKey+"&taskDefName="+taskDefName;
}

function permManage1(taskDefKey,taskDefName)
{
	window.location.href="${ctx}/eform/bind/list?procDefKey=${procDefKey}&processDefinitionId=${processDefinitionId}&procDefName=${procDefName}&taskDefKey="+taskDefKey+"&taskDefName="+taskDefName;
}
</script>
</html>