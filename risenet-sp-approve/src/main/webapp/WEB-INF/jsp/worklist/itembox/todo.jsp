...
<script type="text/javascript">
		...
		function initComplete(){
			size();
			 g = $("#maingrid").quiGrid({
		       columns: [ 
							{ display: '催办信息', name: 'taskIdReminder', align: 'center', width: "10%",
								render: function (rowdata, rowindex, value, column){
		                		    var s = "";
		    						if(value!="")
		    						{
		    							s = '<a href="javascript:void(0);" class="aColor" title="点击查看" onclick="getReminders(\'' + value + '\');">查看</a>';
		    						}else
		    						{
		    							s = '--';
		    						}
							...
							{ display: '状态', name: 'status', align: 'left', width: "5%",
								render: function (rowdata, rowindex, value, column){
									...
									if(rowdata.isNewTodo==1){
										s+= '<img src="${ctx}/static/images/new.gif" title="文件状态：新文件，未阅状态。"/>';
									}
									if($.trim(rowdata.description).length!=0&&rowdata.description.indexOf("退回")>=0){
										s+= '<img src="${ctx}/static/images/rollback.jpg" title="文件状态：'+rowdata.description+'"/>';
									}
									return s;
								}
							},
			                ...
			                { display: '任务名称', name: 'taskName', align: 'center', width: "13%", isSort:false},
			                { display: '开始时间', name: 'taskCreateTime',align: 'center', width: "12%"},
			                ...
			                { display: '发送人', name: 'taskSender',   align: 'center', width: "10%"},//0未回复，1已回复
		         ...
		         url : '${ctx}/sp/worklist/todo/list',rownumbers:true,pageSize:30,percentWidthMode:true,usePager:true,
		         pageSizeOptions:[10, 20, 30, 40, 50],height: '100%', width:"100%",
		         onSuccess:function setPageSize(){
				 		pageNo = "${pageNo}";
				 		pageSize = "${pageSize}";
		         	},
		         toolbar:{
		...
		function process(taskId,documentTitle) {
			var url = '${ctx}/sp/document/edit?itembox=todo&taskId=' + taskId;
		...
		/* function history(processInstanceId,documentTitle) {
			openCurWindow({
				id : frameID,
				src :"${ctx}/sp/worklist/history/show?processInstanceId="+processInstanceId,
				destroy : true,
				title : "历程（"+documentTitle+"）",
				width : winWidth*0.8,
		...
		//查看催办信息
		function getReminders(taskId){
	    	Dialog.open({URL:'${ctx}/reminder/show/' + taskId+'/todo',Title:"催办信息",Width:winWidth*0.8,Height:winHeight*0.8});
		}
	...
	function exportData(isPage){
		...
		var todoDocumentTitle = $("#todoDocumentTitle").val();
		var taskName = $("#taskName").val();
		var taskSender = $("#taskSender").val();
		var startTime = $("#startTime").val();
		var endTime = $("#endTime").val();
        var url = ctx+"/sp/worklist/todoExcel?isPage="+isPage
        		+"&todoDocumentTitle="+todoDocumentTitle
        		+"&taskName="+taskName
...
<body>
	<div class="box2" panelTitle="查询" id="searchPanel">
		...
		<table>
			<tr>
				<td>文件标题：</td>
				<td><input type="text" name="todoDocumentTitle" id="todoDocumentTitle" 
