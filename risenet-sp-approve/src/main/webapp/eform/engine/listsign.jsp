...
String error = net.business.engine.common.Tools.getValidRequest(request);
if(error !=  null)
{
    out.println(error);
    return;
...
String p = net.sysmain.util.StringTools.toHtmlString(request.getParameter("p"));
Operator operator = (Operator) session.getAttribute(net.sysmain.common.I_UserConstant.USER_INFO);
String url = "listsign.jsp?p=" + ((p==null)?"":p);
...
{
    AppDataManager.addOpinionDic(operator, request.getParameter("value"),  p,  request.getParameter("lc"));
}
else
{
    url = null;
...
java.util.ArrayList list = AppDataManager.getOpinionDic(operator, p);

int agent = net.sysplat.access.Authentication.getUserAgentType(request);
String clickEvent = (agent == 0)?" onclick=\"focusLine()\" ondblclick":" onclick";
%>
<html>
<head>
<style>
...
function deleteLine(guid)
{
    ...
    location = "listsign.jsp?p=<%=(p==null)?"":p%>&a=del&guid=" + guid;
}


function setValue(value, lc)
{
    location = "listsign.jsp?lc=" + lc + "&p=<%=(p==null)?"":p%>&a=add&value=" + value;
}
