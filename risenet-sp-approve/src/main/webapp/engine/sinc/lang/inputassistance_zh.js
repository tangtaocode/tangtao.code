...
{   
    ...
    if(objSelect == null ){alert("元素不在form表單內"); return}
    if(objSelect.getAttribute("isCodeItem") == "1") isCodeItem = true;
    if(xmlID == null || xmlID.childNodes == null || xmlID.childNodes.length == null) return;
...
function confirmSubmit(frmObj)
{
    ...
    try
    {
		...
		if(frmObj._button && frmObj._button.tagName =="INPUT") 
        {
			...
			if(_actionName != "確認") msg = "確認" + _actionName + "嗎？";
	    }
    }
    catch(e)
    {
...
function compareDate(name1, o1, name2, ctrlName, type)
{
    ...
    {
        case ">":
          if(v1<=v2){alert(name1+ "必須大于" + name2);return false;}else{break;}
        case ">=":
          if(v1<v2){alert(name1+ "必須大于等于" + name2); return false;}else{break;}
        case "<":
          if(v1>=v2){alert(name1+ "必須小于" + name2);return false;}else{break;}
        case "<=":
          if(v1>v2){alert(name1+ "必須小于等于" + name2);return false;}else{break;}
        default:
          alert("不支持的時間比較方式，請聯系系統管理員");
          return false;
	  }
	  return true;
}
...
function chysoftXMLHttp(url, fun)
{
	...
	this.xmlhttp.onreadystatechange = function()
    {
	    ...
	    {
		    if(typeof(fun) != "function"){alert("非法的回調函數");return;}
			fun(_xmlhttp);
	    }
    }
