...
function initComplete(){
	g = $("#dataBasic").quiGrid({
	    ...
	    columns:[
	        ...
	        { display: '物料编码', name: 'MATERIAL_CODE',     align: 'center', width: "15%",editor: { type: 'text',maxlength:20,tip:'不能超过20个字符' }},

	        { display: '物料名称', name: 'MATERIAL_NAME',    align: 'center', width: "15%",editor: { type: 'text',maxlength:20,tip:'不能超过20个字' }},
	        
	        { display: '当前库存', name: 'MATERIAL_NUM',    align: 'center', width: "10%",editor: { type: 'text',maxlength:20,tip:'不能超过20个字' }},
			
	        { display: '单位', name: 'UNIT',    align: 'center', width: "10%",editor: { type: 'text',maxlength:20,tip:'不能超过20个字' }},

	        { display: '登记人', name: 'DEFINEPERSON',     align: 'center',  width:"15%",editor: { type: 'text',maxlength:20,tip:'不能超过20个字' }} ,
	        
	        
	        { display: '登记时间', name: 'DEFINETIME',     align: 'center',  width:"15%",type:"date",editor: { type: 'date',dateFmt:'yyyy-MM-dd'}} ,
	        
	        { display: '排序号', name: 'TABINDEX',     align: 'center', width: "5%",editor: { type: 'text',maxlength:20}},
...
function onDelete(rowidx){
	Dialog.confirm("确定要删除该记录吗？",function(){
	  	...
	  	$.post("${ctx}/materialManage/materialManageDelete",{"materialguid":row.MATERIALGUID,"material_code":row.MATERIAL_CODE},function(result){
...
function materialadd() {
	window.location.href = "${ctx}/materialManage/materialManageAdd"; 
}

function selectMaterial(){
	Dialog.open({URL:"${ctx}/materialManage/materialDictionaryIndex",Title:"流程管理",Width:500,Height:330});
}
...
function fanhui(){
 ...
 function rowToBO(row) {
    var params = '&materialguid='+row.MATERIALGUID+'&material_code='+row.MATERIAL_CODE+'&material_name='+row.MATERIAL_NAME+'&material_num='+row.MATERIAL_NUM+'&unit='+row.UNIT+'&tabindex='+row.TABINDEX+'&definetime='+row.DEFINETIME;
    return params;
} 
...
function onAfterSubmitEdit(e) {
	...
	rowData.MATERIALGUID = e.record.MATERIALGUID;
    $.post('${ctx}/materialManage/materialManageUpdata',rowToBO(rowData),function(result){         
          Dialog.alert(result.msg);
    },"json");
}
//删除后的提示
