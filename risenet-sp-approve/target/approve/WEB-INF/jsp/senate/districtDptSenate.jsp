...
var personidPage="${personid}districtDptSenate";
</script>
</head>
<body>
	...
	<div class="box2" panelTitle="查询" id="searchPanel">
		...
		<table>
			<tr>
				<td>受理单位：</td>
				<td>
				<select name="bureauguid" id="bureauguid" prompt="请选择" url="${ctx}/senateManage/getSpmBureauList"></select>
				</td>
				<td>评价日期：</td>
...
<script type="text/javascript">
		...
		function initComplete(){
			 g = $("#maingrid").quiGrid({
		       columns: [ 
						...
						{ display: '受理单位', name: 'BUREAUNAME', align: 'center', width: "20%"},
		        ...
		        url:"${ctx}/senateManage/senateList?classify=district&resultType="+personidPage,rownumbers:true,percentWidthMode:true,checkbox:false,
    ...
    function viewDetail(guid,type){
    	Dialog.open({URL:"${ctx}/senateManage/districtDptDetail?bureauGuid="+guid+"&type="+type,Title:"区级部门评价详细信息列表",Width:1200,Height:800});
    }
    
  //导出为Excel
	function exportPageData(){
	  	window.location.href="${ctx}/senateManage/exportData?filename=区级部门评价统计表&resultType="+personidPage;
