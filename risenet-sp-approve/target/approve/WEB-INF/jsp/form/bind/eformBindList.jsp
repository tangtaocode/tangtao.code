	...
	<div class="easyui-layout" data-options="fit:true,border:true">
		<div data-options="region:'center',title:'表单绑定列表(${procDefName}-->${taskDefName})',border:true">
			<div id="toolbar" class="toolbar"  align="left" style="width:100%; background-color:#CCEEFF;">
				<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" id="button01" >电子表单绑定</a>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		grid.datagrid({
			url : '${ctx}/eform/bind/getList?processDefinitionId=${processDefinitionId}&taskDefKey=${taskDefKey}',
			toolbar : '#toolbar',
			fit : true,
			fitColumns : true,
			striped : true,
			...
			columns : [[ {
				...
				formatter : function(cellvalue, rowObject, rowIndex) {
					var s = '';
					s += '<a href="javascript:void(0);" onclick="eformBindDelete(\'' + rowObject.id + '\');">删除</a>';
					s += '&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="eformBindEdit(\'' + rowObject.id + '\');">修改</a>';
					return s;
				}
			} ]]
		});
	...
	function eformBindDelete(id) {
		$( "#dialog-delete" ).dialog({
			...
			buttons:[{
				text:'确定',
				handler:function() {
					$.getJSON('${ctx}/eform/bind/delete?id=' + id, function(data, textStatus, jqXHR) {
						alert(data.msg);
						selectproc();
					});
	...
	var frameID = newGuid();
	function eformBindEdit(id) {
		openCurWindow({
			id : frameID,
			src : '${ctx}/eform/bind/eFormBindEdit?id=' + id +'&processDefinitionId=${processDefinitionId}&procDefName=${procDefName}&taskDefKey=${taskDefKey}&taskDefName=${taskDefName}',
			destroy : true,
			title : '电子表单绑定基本信息',
			width : 600,
			height : 350,
	...
	function reloadgrid(processDefinitionId){
		grid.datagrid({url:'${ctx}/eform/bind/getList',queryParams:{
