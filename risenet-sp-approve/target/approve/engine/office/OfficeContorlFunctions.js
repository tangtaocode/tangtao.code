...
function setFileOpenedOrClosed(bool)
{
    ...
    try{if(opener._getOfficePermitTrace())SetReviewMode(true);OFFICE_CONTROL_OBJ.WebUserName = currentUserName}catch(e){}
    try{if(opener._getOfficePermit() != 2)OFFICE_CONTROL_OBJ.SetReadOnly(true);}catch(e){}
}
function trim(str)
{ //删除左右两端的空格
...
function saveFileToUrl()
{
	...
	if(IsFileOpened)
	{
		...
		alert(result);
    if(result.indexOf("成功保存文档") != -1) try{opener.doRefresh()}catch(e){}
		window.close();
	}
}
...
function doRefresh()
{
    var url = new String(opener.location);
    var pos = url.indexOf("?");
    var arr = [];
    var frm = null;
    ...
    if(arr["ins"] != null)
    {
        url = "gettemplate.jsp?temp_Id=" + arr["temp_Id"] + "&ins=" + arr["ins"];
    }
    else
    {
        url = "gettemplate.jsp?temp_Id=" + arr["temp_Id"] + "&edittype=1&guid=" + frm.guid.value;
    }
...
function addLocalSecSign()
{
	if(IsFileOpened)
	{
		...
		{
			try
			{OFFICE_CONTROL_OBJ.AddSecSignFromLocal(signUser,"");}
			catch(error){}
		}
		else
		{alert("不能在该类型文档中使用安全签名印章.");}
...
function addServerSign(signUrl)
{
	if(IsFileOpened)
	{
			try
			{
          OFFICE_CONTROL_OBJ.AddSecSignFromURL(signUser, signUrl);
				//OFFICE_CONTROL_OBJ.AddSecSignFromURL("ntko",//印章的用户名
...
function addLocalSign()
{
	if(IsFileOpened)
	{
			try
			{
				OFFICE_CONTROL_OBJ.AddSignFromLocal(signUser,//印章的用户名
					"",//缺省文件名
					true,//是否提示选择
					100,//左边距
					100,//上边距 根据Relative的设定选择不同参照对象
					signUser,//调用DoCheckSign函数签名印章信息,用来验证印章的字符串
					3,  //Relative,取值1-4。设置左边距和上边距相对以下对象所在的位置 1：光标位置；2：页边距；3：页面距离 4：默认设置栏，段落
					100,//缩放印章,默认100%
					1);   //0印章位于文字下方,1位于上方
			}
...
function doHandSign()
{
	...
	{*/
		OFFICE_CONTROL_OBJ.DoHandSign2(
									signUser,//手写签名用户名称
									signUser,//signkey,DoCheckSign(检查印章函数)需要的验证密钥。
									0,//left
									0,//top
...
function DoCheckSign()
{
   ...
   {	
			...
			signUser//使用盖章时的signkey,这里为"ntko"
			);//返回值，验证结果字符串
			//alert(ret);
   }	
}
