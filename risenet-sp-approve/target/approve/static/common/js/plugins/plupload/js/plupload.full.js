...
(function() {
	...
	g.File = function(r, p, q) {
		...
		o.url = ""; // 文件url
		o.guid = ""; // 文件guid
		o.uploaderName = ""; // 上传者姓名
		o.uploaderGuid = ""; // 上传者id
		o.uploadDate = new Date().getTime(); // 日期
		o.extName = ""; // 扩展名
		o.isUploadError = false ; //是错上传出错
		o.downloadUrl = "" ;
		o.permission = new g.Permission();//权限
	};
	g.Permission = function() {
		p = this ;
		p.remove = true ;
		p.read = true ;
	}
...
(function(g, b, d, e) {
	...
	function c(o) {
		...
		if (m === "string") {
			n = "\bb\tt\nn\ff\rr\"\"''\\\\";
			return '"' + o.replace(/([\u0080-\uFFFF\x00-\x1f\"])/g, function(r,
					q) {
				var p = n.indexOf(q);
				if (p + 1) {
					return "\\" + n.charAt(p + 1)
