...
public class AccessPushServiceImpl implements IAccessPushService {
	...
	public void pushOrder(CacheObject pushobj) {
		...
		//取该订单号在数据库的主键id补0作为新的订单号推送
		orderFlowAccess.setFlowNo(OrderUtil.getOrderByOrderNo(String.valueOf(order.getId()), 11));
		...
		try {
			...
			String rejson = HttpClientUtils.doBodyPost(accessConfig.getAccessApiOrderFlow(), param, "UTF-8",
	...
	public void pushWxOrder(CacheObject pushobj) {
		...
		param.setOrderNo(pushobj.getOut_trade_no());
		param = orderMapper.selectByParams(param);
		...
		//取该订单号在数据库的主键id补0作为新的订单号推送
		wxorderFlowAccess.setOrderNo(OrderUtil.getOrderByOrderNo(String.valueOf(param.getId()), 11));
		...
		wxorderFlowAccess.setPayTime(DateUtils.date2String(param.getCreateDate(), "yyyy-MM-dd HH:mm:ss"));
		String type = DreamStatus.AL.equals(param.getPaymentType()) ? "支付宝" : "微信";
