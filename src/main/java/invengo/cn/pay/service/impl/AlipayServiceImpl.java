...
import invengo.cn.pay.config.ALconfig;
...
import invengo.cn.pay.model.ali.AlGoodsDetail;
import invengo.cn.pay.model.ali.NotifyResponse;
import invengo.cn.pay.model.ali.OrderRequest;
...
public class AlipayServiceImpl extends BaseComponent implements IAlipayService {
	...
	public DreamResponse unifiedOrder(IPayDTO dto) {
		...
		try {
			...
			mapdto.put("out_trade_no", order.getOut_trade_no());
			mapdto.put("device_info", real.getMac());
			mapdto.put("payment_type", DreamStatus.AL);
			...
			if (response.isSuccess()) {
				...
				cache.setDevice_info(mapdto.get("device_info") + "");
	...
	private StockVO getOrder(CommonDTO real) {
		...
		res.setStore_id(stockVo.getListStock().get(0).getWarehouseId() + "");
