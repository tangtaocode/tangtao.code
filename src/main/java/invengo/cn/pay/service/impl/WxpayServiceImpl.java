...
public class WxpayServiceImpl implements IWxpayService
{
  ...
  public DreamResponse unifiedOrder(IPayDTO dto)
  {
    ...
    try {
        ...
        if(DreamStatus.SUCCESS.equals(resmap.get("return_code"))&&DreamStatus.SUCCESS.equals(resmap.get("result_code"))){
	...
	public DreamResponse orderQuery(IPayDTO dto) {
        ...
        try {
            ...
            //根据trade_state判断交易状态,当且仅当return_code 、result_code、trade_state都为SUCCESS时返回订单的所有数据
  ...
  public String notify(IPayDTO dto)
  {
    ...
    try
    {
      WxPayDTO real = (WxPayDTO)dto;
      WXPay wxpay = new WXPay(wxconfig);
      logger.info("支付宝通知成功："+new Gson().toJson(real)); 
      ...
      if(wxpay.isPayResultNotifySignatureValid(BeanUtil.objectToMapStr(real))){
          //签名正确
          logger.info("微信验签成功："+new Gson().toJson(real));
          if(DreamStatus.SUCCESS.equals(real.getResult_code())&&DreamStatus.SUCCESS.equals(real.getReturn_code())){
